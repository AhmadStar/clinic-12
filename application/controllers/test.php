...
class Test extends CI_Controller {
  ...
  public function search()
  {
    ...
    if($this->input->post())
    {
        $this->load->model('lab');
        $q=$this->input->post('q');
        $lab=$this->lab->search(array('test_name_en'=>$q,'test_name_ar'=>$q));
        $data['lab']=$lab;
        $this->load->view('lab/result',$data);
        return TRUE;
    }
    $data['title']=tr('TestSearch');
  ...
  public function edit($test_id=0)
  {
    ...
    if($this->input->post())
    {
      $this->form_validation->set_rules(array(
        ...
        array( 'field' => 'test_name_ar', 'label' => 'Test Name in Dari', 'rules' => 'required|trim|has_no_schar', ),
