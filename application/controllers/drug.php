...
class Drug extends CI_Controller {
  ...
  public function search(/*$q=''*/)
  {
    ...
    if($this->input->post())
    {
        $this->load->model('drugs');
        $q=$this->input->post('q');
        //$drugs=$this->drugs->search(array('drug_name_en'=>$q,'drug_name_ar'=>$q));
        $drugs=$this->drugs->search(array('drug_name_en'=>$q,'drug_name_ar'=>$q));
        $data['drugs']=$drugs;
  ...
  public function edit($drug_id=0)
  {
    ...
    if($this->input->post())
    {
      $this->form_validation->set_rules(array(
        ...
        array( 'field' => 'drug_name_ar', 'label' => 'Drug Name in Arabic', 'rules' => 'required|trim|has_no_schar', ),
  ...
  public function _drugs_list()
  {
    ...
    $drugs_list['0']= tr('DrugName');
    foreach ($drugs as $drug) 
    {
      $drugs_list[$drug->drug_id]=  html_escape($drug->drug_name_ar.', '.$drug->drug_name_en.', '.$drug->price);
    }
    return $drugs_list;
  }
