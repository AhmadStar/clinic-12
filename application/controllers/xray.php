...
class Xray extends CI_Controller {
  ...
  public function search()
  {
    ...
    if($this->input->post())
    {
        $this->load->model('xrays');
        $q=$this->input->post('q');
        $xrays=$this->xrays->search(array('xray_name_en'=>$q,'xray_name_ar'=>$q));
        $data['xrays']=$xrays;
        $this->load->view('xray/result',$data);
        return TRUE;
    }
    $data['title']=tr('XraySearch');
  ...
  public function edit($xray_id=0)
  {
    ...
    if($this->input->post())
    {
      $this->form_validation->set_rules(array(
        ...
        array( 'field' => 'xray_name_ar', 'label' => 'Xray Name in Dari', 'rules' => 'required|trim|has_no_schar', ),
  ...
  public function new_xray()
  {
    ...
    $data['title'] = tr('RegisterNewX-ray');    
