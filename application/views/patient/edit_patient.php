...
<div class="col col-md-8 well well-md">
  ...
  <?php echo (!empty($error) ? $error : '' ); ?>
    <fieldset>
      <legend>- <?php trP('PatientInformation')?>:</legend>
      <div>
        <div class="form-group">
          <div class="col-md-9">
            <div><input type="text" name='first_name' id="first_name" value="<?php echo set_value('first_name',$patient->first_name);?>" class='form-control' placeholder="<?php trP('FirstName')?>" title='First Name' required autofocus /></div>
            <div><input type="text" name='last_name' id='last_name' value="<?php echo set_value('last_name',$patient->last_name);?>" class='form-control' placeholder="<?php trP('LastName')?>" title='Last Name' /></div>
            <div><input type="text" name='fname' id='fname' value="<?php echo set_value('fname',$patient->fname);?>" class='form-control' placeholder="<?php trP('FatherName')?>" title='Father Name' /></div>
            <div class="col-md-12" style="margin-bottom:10px;">
              <label class="radio-inline"><input type="radio" name='gender' value="1" title='Male' <?php echo isset($_POST['gender'])?($this->input->post('gender')?'checked':''):($patient->gender?'checked':'');?> /><?php trP('Male')?></label>
              <label class="radio-inline"><input type="radio" name='gender' value="0" title='Female' <?php echo isset($_POST['gender'])?($this->input->post('gender')?'':'checked'):($patient->gender?'':'checked');?> /><?php trP('Female')?></label>
            </div>
          </div>
          <div class="col-md-3">
    ...
    <fieldset>
      ...
      <div>
        <div class="form-group">
          <div class="col-md-6"><input type='email' name='email' id='email' value="<?php echo set_value('email',$patient->email);?>" class='form-control' placeholder="<?php trP('Email')?>" title='Email' /></div>
        </div>
        <div class="form-group">
          <div class="col-md-6"><input type='phone' name='phone' id='phone' value="<?php echo set_value('phone',$patient->phone);?>" class='form-control' placeholder="<?php trP('Phone')?>" title='Phone' required/></div>
        </div>        
        <div class="clearfix"></div>
        <div class="form-group">
          <div class="col-md-12"><input type="text" name='address' id='address' value="<?php echo set_value('address',$patient->address);?>" class='form-control' placeholder="<?php trP('Address')?>" title='Address'/></div>
        </div>
        <div class="form-group">
          <div class="col-md-6"><input type="text" name='social_id' id='social_id' value="<?php echo set_value('social_id',$patient->social_id)?>" class='form-control' placeholder="<?php trP('SocialID')?>" title='Social ID'/></div>
        </div>
        <div class="form-group">
          <div class="col-md-6">
    ...
    <div class="form-group">
     <div class="col-md-6"><input type="submit" name='submit' id='submit' value=<?php trp('update')?> class="form-control btn btn-info" /></div>
        <div class="col-md-6"><?php echo anchor('patient',tr('cancel'),array('class'=>'form-control btn btn-info'));?></div>      
    </div>
  <?php echo form_close(); ?>
</div>
<?php 
