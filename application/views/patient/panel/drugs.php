    ...
    if(@$drugs){
      ...
      foreach ($drugs as $drug) {
        ...
        if(!($drug->user_id_discharge&&$drug->discharge_date))
        {
            ...
            echo '<td class="actions">'.anchor('#', tr('Delete'),array('dpi'=>$drug->drug_patient_id,'di'=>$drug->drug_id,'pi'=>$drug->patient_id,'action'=>'delete',/*'onclick'=>'drugItemsAction',*/'tc'=>$drug->total_cost));
            echo '  ';
            if($this->bitauth->has_role('receptionist'))
            { 
                echo anchor('#', tr('Pay'),array('dpi'=>$drug->drug_patient_id,'di'=>$drug->drug_id,'pi'=>$drug->patient_id,'action'=>'pay',/*'onclick'=>'drugItemsAction',*/'tc'=>$drug->total_cost));
            }else{
                echo '</td>';
            }
      ...
      echo '<tr class="paid text-success '.($paid?'':'hidden').'"><td></td><td></td><td></td><td></td><td>'.tr('Paid').':</td><td id="paid">'.$paid.'</td><td></td></tr>';
      echo '<tr class="unpaid text-danger '.($unpaid?'':'hidden').'"><td></td><td></td><td></td><td></td><td>'.tr('Unpaid').':</td><td id="unpaid">'.$unpaid.'</td><td></td></tr>';
      echo '<tr class="tc text-info '.($paid&&$unpaid?'':'hidden').'"><td></td><td></td><td></td><td></td><td>'.tr('Total').':</td><td id="tc">'.($paid+$unpaid).'</td><td></td></tr>';
    }?>
    </tbody></table></div>
</div>